...
import re as _backup_re

def _validate_backup_id(backup_id):
    """Validate backup_id contains only safe characters."""
    if not backup_id or not isinstance(backup_id, str):
        return False
    if not _backup_re.match(r'^[a-zA-Z0-9_\-\.]+$', backup_id):
        return False
...
class BackupManager:
    ...
    def restore_backup(self, backup_id):
        """Restore a backup"""
        try:
            # SECURITY: Defense-in-depth validation of backup_id
...
def restore_backup():
    ...
    try:
        ...
        # SECURITY: Validate backup_id to prevent path traversal
        if not _validate_backup_id(backup_id):
            logger.warning(f"Invalid backup_id rejected: {backup_id}")
            return jsonify({"success": False, "error": "Invalid backup ID format"}), 400
...
def delete_backup():
    ...
    try:
        ...
        if not _validate_backup_id(backup_id):
            logger.warning(f"Invalid backup_id rejected for deletion: {backup_id}")
            return jsonify({"success": False, "error": "Invalid backup ID format"}), 400
...
def download_backup(backup_id):
    ...
    try:
        # SECURITY: Validate backup_id to prevent path traversal
        if not _validate_backup_id(backup_id):
            logger.warning(f"Invalid backup_id rejected for download: {backup_id}")
            return jsonify({"success": False, "error": "Invalid backup ID format"}), 400
...
def upload_backup():
    ...
    try:
        ...
        # Extract ZIP file with path sanitization (Zip Slip protection)
        with zipfile.ZipFile(temp_zip_path, 'r') as zipf:
            for member in zipf.namelist():
                ...
                if member_path.startswith('..') or member_path.startswith('/') or '..' in member_path:
                    ...
                    raise Exception(f"Invalid path in zip file: {member}")
                target_path = os.path.join(str(temp_dir), member_path)
                if not os.path.abspath(target_path).startswith(os.path.abspath(str(temp_dir))):
                    logger.error(f"Path traversal attempt: {member} -> {target_path}")
                    raise Exception(f"Path traversal attempt blocked: {member}")
                if member.endswith('/'):
