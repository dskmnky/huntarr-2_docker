...
# Argon2 optional - install argon2-cffi for stronger hashing
try:
...
def _password_is_hashed(stored: str) -> bool:
    """True if stored value looks like a hash (not plaintext). Supports Argon2, bcrypt and salt:hash."""
    if not stored or len(stored) < 10:
        return False
    if stored.startswith("$argon2"):
        return True  # Argon2id (new format)
...
# Argon2id hasher with secure defaults (OWASP recommendations)
_argon2_hasher = PasswordHasher(time_cost=3, memory_cost=65536, parallelism=4, hash_len=32, salt_len=16) if ARGON2_AVAILABLE else None


def hash_password(password: str) -> str:
    """Hash a password for storage. Uses Argon2id if available, else SHA-256."""
    if ARGON2_AVAILABLE and _argon2_hasher:
...
def verify_password(stored_password: str, provided_password: str) -> bool:
    ...
    Supports: Argon2id (new), salt:hash (SHA-256 legacy), bcrypt, and plaintext (for migration).
    """
    if not stored_password or not provided_password:
        return False
    # Argon2id (new format - starts with $argon2)
    if ARGON2_AVAILABLE and stored_password.startswith("$argon2"):
        try:
            _argon2_hasher.verify(stored_password, provided_password)
            return True
    ...
    if stored_password.startswith("$2") and stored_password.count("$") >= 3:
        try:
            ...
            return False
    # Salt:hash (our SHA-256 format) - legacy, will be rehashed on next password change
...
def authenticate_request():
    ...
    # Skip authentication for static files, API setup (except clear), health check, ping, sponsors, version
    # SECURITY: /api/setup/clear requires authentication to prevent unauthorized resets
    is_setup_public = request.path.startswith('/api/setup') and not request.path.endswith('/clear')
    if request.path.startswith('/static/') or is_setup_public or request.path.endswith('/favicon.ico') or request.path.startswith('/api/health') or request.path.endswith('/ping')  or request.path.endswith('/api/version'):
        return None

    # Skip authentication for login pages and specific Plex OAuth endpoints only
    # SECURITY: Be very selective - only allow truly public endpoints
    # /api/auth/plex/init and /api/auth/plex/callback needed for OAuth flow
    # Removed: /api/settings/general, /api/user/2fa/, /auth/recovery-key (all require auth now)
    plex_public_endpoints = ['/api/auth/plex/init', '/api/auth/plex/callback', '/api/auth/plex/status']
    is_plex_public = any(request.path.endswith(ep) for ep in plex_public_endpoints)
    if request.path.endswith('/login') or request.path.startswith('/api/login') or is_plex_public:
        if not is_polling_endpoint:
            if hash(request.path) % 20 == 0:
                logger.debug(f"Skipping authentication for login/plex-oauth path '{request.path}'")
        return None
    ...
    if local_access_bypass:
        ...
        # Check if request is coming through a TRUSTED proxy
        # SECURITY: Only trust X-Forwarded-For if the direct connection is from localhost/Docker
        # This prevents external attackers from spoofing local IPs via the header
        trusted_proxy_sources = ['127.0.0.1', '::1', '172.17.', '172.18.', '172.19.', '172.20.', '172.21.']
        is_from_trusted_proxy = any(
            remote_addr == src or (src.endswith('.') and remote_addr.startswith(src))
            for src in trusted_proxy_sources
        )
        
        forwarded_for = request.headers.get('X-Forwarded-For')
        if forwarded_for and is_from_trusted_proxy:
            logger.debug(f"X-Forwarded-For header from trusted proxy: {forwarded_for}")
            ...
            logger.warning(f"Ignoring X-Forwarded-For from untrusted source {remote_addr}")
...
def create_plex_pin(setup_mode: bool = False, user_mode: bool = False) -> Optional[Dict[str, Union[str, int]]]:
    ...
    try:
        response = requests.post('https://plex.tv/api/v2/pins', headers=headers, data=data, timeout=30)
...
def check_plex_pin(pin_id: int) -> Optional[str]:
    ...
    try:
        response = requests.get(f'https://plex.tv/api/v2/pins/{pin_id}', headers=headers, params=data, timeout=30)
        response.raise_for_status()
...
def verify_plex_token(token: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        response = requests.get('https://plex.tv/api/v2/user', headers=headers, timeout=30)
