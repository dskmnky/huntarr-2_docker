...
def _redact_settings(settings: dict) -> dict:
    """Redact sensitive values from settings dict for API responses.
    
    Wrapper around settings_manager.redact_secrets for use in web routes.
    Prevents credential leakage (API keys, passwords, tokens) in API responses.
    """
    from src.primary.settings_manager import redact_secrets
...
# Security: Set max upload size (100MB) and session cookie flags
...
def api_settings():
    if request.method == 'GET':
        ...
        try:
            ...
            pass
        # SECURITY: Redact sensitive data (API keys, passwords, tokens) before returning
        return jsonify(_redact_settings(all_settings))
...
def save_general_settings():
    ...
    if success:
        ...
        # Return all settings with sensitive data redacted
        # SECURITY: Don't expose API keys in responses
        return jsonify(_redact_settings(settings_manager.get_all_settings()))
    else:
...
def handle_app_settings(app_name):
    ...
    if request.method == 'GET':
        ...
        return jsonify(_redact_settings(app_settings))
        ...
        if success:
            ...
            return jsonify({"success": True, "settings": _redact_settings(updated)})
        else:
...
def api_reset_settings():
    ...
    if success:
        ...
        all_settings = settings_manager.get_all_settings()
        # SECURITY: Redact sensitive data from response
        return jsonify(_redact_settings(all_settings))
    else:
...
def api_app_settings():
    ...
    # SECURITY: Don't expose raw API keys in responses - only return whether they're configured
    api_url = settings_manager.get_api_url(app_type)
    api_key = settings_manager.get_api_key(app_type)
    api_details = {
        "api_url": api_url,
        "api_key": "***REDACTED***" if api_key else ""  # Don't expose actual API key
    }
