...
def handle_oauth():
    ...
    try:
        ...
        response = requests.post('https://plex.tv/api/v2/oauth/token', 
                               data=token_data, headers=headers, timeout=30)
...
def link_account():
    ...
    try:
        ...
        # SECURITY: Don't trust client-provided setup_mode - verify server-side
        from src.primary.utils.database import get_database
        db = get_database()
        setup_mode = client_setup_mode and db.is_setup_in_progress()
        
        if client_setup_mode and not setup_mode:
            logger.warning("Client claimed setup_mode but server setup is not in progress")
