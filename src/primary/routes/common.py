from src.primary.cors_helper import get_cors_origin, add_cors_headers
#!/usr/bin/env python3
"""
...
def api_get_sleep_json():
    ...
    try:
        ...
        origin = get_cors_origin(); response.headers['Access-Control-Allow-Origin'] = origin if origin else request.host_url.rstrip('/')
        return response
...
def setup_2fa():
    ...
    # SECURITY: Only allow 2FA setup from authenticated sessions or during initial setup
    # Removed get_first_user() fallback - it allowed unauthenticated 2FA manipulation
    if not username:
        try:
            from src.primary.utils.database import get_database
            db = get_database()
            # Only trust setup_progress if setup is actually in progress (server-side check)
            if db.is_setup_in_progress():
                setup_progress = db.get_setup_progress()
                if setup_progress and setup_progress.get('username'):
                    username = setup_progress.get('username')
                    logger.debug(f"Using username from active setup progress: {username}")
        except Exception as e:
            logger.error(f"Error checking setup status for 2FA setup: {e}")

    if not username:
        logger.warning("2FA setup attempt failed: Authentication required")
...
def verify_2fa():
    ...
    # SECURITY: Only allow 2FA verify from authenticated sessions or during initial setup
    # Removed get_first_user() fallback - it allowed unauthenticated 2FA manipulation
    if not username:
        try:
            from src.primary.utils.database import get_database
            ...
            logger.error(f"Error checking setup status for 2FA verify: {e}")

    if not username:
        logger.warning("2FA verify attempt failed: Authentication required")
        return jsonify({"error": "Authentication required"}), 401
...
def generate_recovery_key():
    ...
    # SECURITY: Track if we authenticate via validated setup mode
    _validated_setup_mode = False
    # If not authenticated, check if we're in setup mode and get username from setup progress
    # SECURITY: Verify setup_mode server-side, don't trust client
    if not username:
        try:
            data = request.json or {}
            client_setup_mode = data.get('setup_mode', False)
            from ..utils.database import get_database
            db = get_database()
            # Only trust setup_mode if server confirms setup is actually in progress
            setup_mode = client_setup_mode and db.is_setup_in_progress()
            if client_setup_mode and not setup_mode:
                logger.warning("Recovery key: Client claimed setup_mode but server setup is not in progress")
            if setup_mode:
                setup_progress = db.get_setup_progress()
                if setup_progress and setup_progress.get('username'):
                    username = setup_progress['username']
                    logger.debug(f"Using username from setup progress: {username}")
                    _validated_setup_mode = True  # SECURITY: Authenticated via validated setup
                else:
    ...
    try:
        ...
        # SECURITY: NEVER trust client setup_mode - use server-validated flag
        # REMOVED: setup_mode = data.get("setup_mode", False)  # Was vulnerable to client manipulation

        # During setup mode, skip password verification
        if not _validated_setup_mode:
        ...
        if recovery_key:
            logger.info(f"Recovery key generated successfully for user: {username} (setup_mode: {_validated_setup_mode})")
            return jsonify({
                "success": True, 
