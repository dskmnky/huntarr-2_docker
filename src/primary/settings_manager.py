...
# Keys that should be redacted in API responses to prevent credential leakage
SENSITIVE_KEYS = ['api_key', 'apikey', 'password', 'secret', 'token', 'credentials', 'dev_key']

def redact_secrets(settings: dict) -> dict:
    """Recursively redact sensitive values from settings dict.
    
    Args:
        settings: Dictionary of settings that may contain sensitive data
    ...
    for key, value in settings.items():
        if isinstance(value, dict):
            ...
            redacted[key] = [redact_secrets(i) if isinstance(i, dict) else i for i in value]
        elif any(sens in key.lower() for sens in SENSITIVE_KEYS):
            # Only redact if there is actually a value
...
def get_all_settings_redacted() -> Dict[str, Dict[str, Any]]:
    """Load settings for all known apps with sensitive data redacted.
    
    Use this for API responses to prevent credential leakage.
    """
    return redact_secrets(get_all_settings())
