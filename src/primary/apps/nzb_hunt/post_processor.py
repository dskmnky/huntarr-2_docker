...
def _extract_zip(zip_path: str, output_dir: str) -> Tuple[bool, str]:
    ...
    Includes path traversal protection to prevent zip slip attacks.
    """
    import zipfile
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            ...
            for member in zf.namelist():
                ...
                # Reject paths that try to escape the output directory
                if member_path.startswith('..') or os.path.isabs(member_path):
                    raise ValueError(f"Unsafe path in archive: {member}")
                target_path = os.path.join(output_dir, member_path)
                if not os.path.abspath(target_path).startswith(os.path.abspath(output_dir)):
        ...
        logger.error(f"ZIP security check failed: {e}")
